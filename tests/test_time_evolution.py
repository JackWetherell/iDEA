...
class Groundstate:
    ...
    def get_analytic_groundstate(params, spin):
        ...
        def potential(N, dx, system, e, w):
            return (
                0.5 * (w**2) * (np.linspace(-0.5 * dx * N, 0.5 * dx * N, num=N)) ** 2
                + e * np.linspace(-0.5 * dx * N, 0.5 * dx * N, num=N)
                + e**2 / (2 * w**2)
            )
        ...
        def hermite(s, w, N, dx):
            if s == 0:
                ...
                return 2 * np.sqrt(w) * np.linspace(-0.5 * dx * N, 0.5 * dx * N, num=N) * hermite(
                    s - 1, w, N, dx
                ) - 2 * (s - 1) * hermite(s - 2, w, N, dx)

        # Function that creates the analytical solution for the QHO for k = 0,1,2 states
        def analytic_solution(
            t, e, w, N, hermite_array, factorial_array, analytic_terms, dx, K
        ):  # Analytic solution for the SHO
            a = -e / (2 * np.sqrt(w) ** 3)
            s = np.linspace(0, analytic_terms, (analytic_terms + 1)).astype(int)
            if K == 0:
                return (
                    ((w / np.pi) ** 0.25)
                    * np.exp((-0.5 * w * (np.linspace(-0.5 * dx * N, 0.5 * dx * N, num=N)) ** 2 + 1.0j * t) - a**2)
                    * np.sum((1.0 / factorial_array[s]) * (a**s) * hermite_array[s] * np.exp(-1.0j * s * w * t))
                )
            if K == 1:
                return (
                    ((1 / np.sqrt(2**K * math.factorial(K))) * (w / np.pi) ** 0.25)
                    * np.exp(-0.5 * w * ((np.linspace(-0.5 * dx * N, 0.5 * dx * N, num=N)) ** 2 + 1.0j * t) - a**2)
                    * np.sum(
                        (1.0 / factorial_array[s])
                        * (s - 2 * a**2)
                        * (a ** (s - K))
                        * hermite_array[s]
                        * np.exp(-1.0j * s * w * t)
...
class TestShort:
    ...
    def test_wavefunction(self, analytic_short, iDEA_short):
        ...
        analytic_wavefunction = np.interp(
            x, np.linspace(x_min, x_max, len(analytic_short.wavefunction[0])), analytic_short.wavefunction[0]
        )
